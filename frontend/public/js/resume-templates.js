...
(function (global) {
  ...
  function renderResume(data, id, opts) {
    ...
    switch (id) {
      ...
      case 'modern_simple':     return apply(tMinimal(data, ac, fo, sp));
      case 'modern_with_photo': return apply(tPhotoModern(data, ac, fo, sp));
      case 'chronological':     return apply(tClassic(data, ac, fo, sp));
      case 'functional':        return apply(tMinimal(data, ac, fo, sp));
      case 'hybrid':            return apply(_doubleCol(data, ac || '#1e3a5f', fo, sp, '#334155', '#fff', false));
      case 'creative':          return apply(tTimeline(data, ac, fo, sp));
      case 'simple_ats':        return apply(tClean(data, ac, fo, sp));
      case 'two_col_ats':       return apply(_doubleCol(data, '#475569', fo, sp, '#f1f5f9', '#1e293b', false));
      case 'polished':          return apply(tElegant(data, ac, fo, sp));
      case 'minimalist':        return apply(tMinimal(data, ac, fo, sp));
      case 'elegant':           return apply(tElegant(data, ac, fo, sp));
      case 'teenager':          return apply(tSimple(data, ac, fo, sp));
      case 'internship':        return apply(tSimple(data, ac, fo, sp));
      case 'entry_level':       return apply(tClean(data, ac, fo, sp));
      case 'career_change':     return apply(tMinimal(data, ac, fo, sp));
